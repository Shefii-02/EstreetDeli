    ...
    <div class="modal fade" id="ProductUpsell" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg product-upsell-modal">
            <div class="modal-content content-body">
...
<script>
    ...
    async function fixCheckedComboItems(){
        ...
        await $('.combo-category').each(async function(){
            ...
            if($(this).find('.combo-check:checked').length){
            ...
            $(this).find('.dropdown-toggle').text(selected);
    ...
    @if(isset($hasNonUpsellItems) && $hasNonUpsellItems)
        @if(!session('success'))
        $.ajax({
            url: '{{url('cart/upsell')}}',
            type: 'GET',
        ...
        $(document).on('click', '.upsell-check', function(){
           setUpsellPrices(); 
        });
        @endif
        async function setUpsellPrices(){
            ...
            $('.up-item').each(async function(){
               ...
               if($(this).find('.upsell-check:checked').length){
                   ...
                   let $discountAmt = ($price+$addonPrices)*($discount/100);
                   let $final = ($price+$addonPrices)-$discountAmt;
                   $(this).find('.effective-price').text('$'+formatNumber($final))
