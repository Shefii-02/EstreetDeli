        ...
        foreach($data->data as $item){
            ...
            if($result != null || $item->action == 'DELETE'){
               ...
               if($item->function == 'Banner'){
                       ...
                       if(!$bnr){
                           ...
                           if($result->data->file_type == 'video'){
                               ...
                               if($image_name = store_image_url("images/banners/",$result->data->picture)){
                                   $image_name2         = store_image_url("images/banners/",$result->data->picture_small);
                                   DeleteImage($bnr->picture,'images/banners/');
                                   $bnr->picture        = $image_name;
                ...
                elseif($item->function == 'Product'){
                   ...
                   if($item->action == 'UPDATE' || $item->action == 'CREATE'){
                       ...
                       $product->has_customization        = $result->data->has_customization;
                        ...
                        $product->combo_product =  $result->data->combo_product;
                       ...
                       try{
                           $product->save();
                            ...
                            ProductCondiment::where('product_id',$product->id)->delete();
                            
                            \App\Models\ComboProduct::whereProductId($product->id)->delete();
                            \App\Models\ProductComboCategory::whereProductId($product->id)->delete();
                            \App\Models\ProductCombo::whereProductId($product->id)->delete();
                            
                            \App\Models\UpsellProduct::whereProductId($product->id)->delete();
                            \App\Models\ProductUpsellCategory::whereProductId($product->id)->delete();
                            \App\Models\ProductUpsell::whereProductId($product->id)->delete();
                            
                            if($result->data->combo){
                                $procombo = new \App\Models\ProductCombo;
                                $procombo->product_id = $product->id;
                                $procombo->price = $result->data->combo->price;
                                $procombo->min_addon = $result->data->combo->min_addon;
                                $procombo->max_addon = $result->data->combo->max_addon;
                                $procombo->name = $result->data->combo->name;
                                $procombo->description = $result->data->combo->description;

                                if($pic = store_image_url("images/products/",$result->data->combo->picture_url)) {
                                    $procombo->picture = $pic;
                                }
                                $procombo->save();
                            }
                            
                            if(count($result->data->combo_categories) > 0){
                                foreach($result->data->combo_categories as $cat)
                                {   
                                    $category = Category::whereMasterId($cat->category_id)->first();
                                    $combocat             =   new \App\Models\ProductComboCategory;
                                    $combocat->product_id =   $product->id;
                                    $combocat->category_id = $category->id;
                                    $combocat->min_addon = $cat->min_addon;
                                    $combocat->max_addon = $cat->max_addon;
                                    $combocat->save();
                                }
                            }
                            
                            if(count($result->data->combo_products) > 0){
                                foreach($result->data->combo_products as $prod)
                                {   
                                    $comboprod = Product::whereMasterId($prod->combo_product_id)->first();
                                    $category = Category::whereMasterId($prod->category_id)->first();
                                    $combop             =   new \App\Models\ComboProduct;
                                    $combop->product_id =   $product->id;
                                    $combop->category_id = $category->id;
                                    $combop->combo_product_id = $comboprod->id;
                                    $combop->save();
                            ...
                            if($result->data->upsell){
                                $proupsell = new \App\Models\ProductUpsell;
                                $proupsell->product_id = $product->id;
                                $proupsell->discount = $result->data->upsell->discount;
                                $proupsell->save();
                            }
                            
                            
                            
                            if(count($result->data->upsell_categories) > 0){
                                foreach($result->data->upsell_categories as $cat)
                                {   
                                    $category = Category::whereMasterId($cat->category_id)->first();
                                    if($category){
                                        $upsellcat             =   new \App\Models\ProductUpsellCategory;
                                        $upsellcat->product_id =   $product->id;
                                        $upsellcat->category_id = $category->id;
                                        $upsellcat->save();
                                    }
                                }
                            }
                            
                            if(count($result->data->upsell_products) > 0){
                                foreach($result->data->upsell_products as $prod)
                                {   
                                    $upsellprod = Product::whereMasterId($prod->upsell_product_id)->first();
                                    $category = Category::whereMasterId($prod->category_id)->first();
                                    if($upsellprod && $category){
                                        $upsellp             =   new \App\Models\UpsellProduct;
                                        $upsellp->product_id =   $product->id;
                                        $upsellp->category_id = $category->id;
                                        $upsellp->upsell_product_id = $upsellprod->id;
                                        $upsellp->save();
                                    }
                        ...
                        if(count($result->data->product_city) > 0){
                          ...
                          if(count($result->data->specializations) > 0){
                            ...
                            if(count($result->data->product_ingredients) > 0){
                                foreach($result->data->product_ingredients as $IngListing){
                                        $product_ingg       = Product::where('master_id',$IngListing->product_id)->first();
                                        if($product_ingg){
                                            $p_sug              =  new ProductIngredient();
                                            $p_sug->name        =  $IngListing->name;
                                            $p_sug->product_id  =  $product_ingg->id;
                                            $p_sug->save();
                                        }
                                }
                            }
                            ...
                            if(count($result->data->product_condiment) > 0){
                                foreach($result->data->product_condiment as $CondListing){
                                        $product_cond       = Product::where('master_id',$result->data->id)->first();
                                        if($product_cond){
                                            $p_sug                  = new ProductCondiment();
                                            $p_sug->condiment_name  = $CondListing->name;
                                            $p_sug->product_id      =  $product_cond->id;
                                            $p_sug->condiment_master_id= $CondListing->id;
                                            $p_sug->save();
                                        }
                                }
                            }
                ...
                elseif($item->function == 'Delivery Partner'){
                     if($item->action == 'CREATE' || $item->action == 'UPDATE'){
                       $newone                = DeliveryPartner::where('master_id',$result->data->id)->first();
                       if(!$newone){
                           $newone             = new DeliveryPartner;
                           $newone->master_id  = $result->data->id;
                       }
                       
                       $newone->name           = $result->data->name;
                       $newone->status           = $result->data->status;
                       $newone->store_link           = $result->data->store_link;
                   ...
                   if($item->action == 'DELETE'){
                      $cnty = DeliveryPartner::where('master_id',$item->data_id)->first();
                      if($cnty){
                          $cnty->delete();
                      }
                   }
    ...
    try {
        if ($url != '') {
            ...
            if ($httpCode === 200 && $imageData !== false) {
                ...
                if (!$image_name) {
                    $image_name = Str::random(30) . '.png'; // Assuming you have the Str class available for generating random strings
                }
