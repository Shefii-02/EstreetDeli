...
class FrontendController extends Controller
{
    ...
    public function index(Request  $request)
    {
        ...
        $slider = Banner::whereType('home_slider')->where('status', 1)->orderBy('display_order','ASC')->get();
        
        $home_tiles = Banner::whereType('home_tiles')->where('status', 1)->orderBy('display_order','ASC')->get();
    						
        $career_stores = Store::orderBy('display_order','asc')->get();
          
        $front_products = HomepageProduct::with('product_list')->first();
        
        $blogs         = Blog::orderBy('published_at','DESC')->whereType('blog')->limit('4')->get();

    	return view('frontend.index',compact('slider','front_products','career_stores','home_tiles','blogs'));
    }
    ...
    public function stores(){
        ...
        return view('frontend.stores',compact('branches'));
    }
    ...
    public function blogs()
    {
        ...
        return view('frontend.blogs',compact('blog_category','blogs'));
    }
    ...
    public function blog_single($single)
    {
        ...
        $title = 'Sweetiepie-'.$single_blog->name;
        $description = Str::limit(strip_tags($single_blog->description), 160); 
        
        return view('frontend.blog-single',compact('blogs','single_blog','blog_category','title','description'));
    }
    ...
    public function faq()
    {
        ...
        return view('frontend.faq',compact('general','substitutions','discounts','returns'));
    }
    ...
    public function order_inquiry(Request $request){
        ...
        return view('frontend.order-inquiry', compact('order_details'));
    ...
    public function share_feedback(Request $request){
        ...
        if($exist){
            ...
            return view('frontend.share-feedback', compact('order_details'));
        }
    }
